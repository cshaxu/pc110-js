...
- PCx86 source generation selected for the golden baseline: v2
...
This commit is the latest fetched PCjs upstream `origin/master` commit at the time the pc110-js standalone direction was recorded. M1 must prove an unmodified PCx86 v2 DOS boot from this commit before source migration. PCjs-derived work must cite this baseline unless a later baseline record supersedes it.

PCx86 v2 defines supported CPU models through the 80386. The generic golden machine must use a supported 80386 configuration. PC110-required 486SX/SL behavior is a separately tested project delta, not an implied upstream 80486 implementation.
