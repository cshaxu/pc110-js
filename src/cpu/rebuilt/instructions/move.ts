...
export function executeMoveModRm(context: RebuiltExecutionContext): void {
  const opcode = context.instruction.opcode;
  if (opcode < 0x88 || opcode > 0x8b)
    throw new Error(`Opcode 0x${opcode.toString(16)} is outside rebuilt MOV coverage`);
  const width: ArithmeticWidth =
    opcode === 0x88 || opcode === 0x8a ? 8 : context.instruction.prefixes.operandSize;
  ...
  if (opcode === 0x88 || opcode === 0x89)
    writeRm(context, modRm, width, readRegister(context, modRm.reg, width));
  else writeRegister(context, modRm.reg, width, readRm(context, modRm, width));
