...
describe("80386 instruction fetch", () => {
  ...
  it("selects Group 1 immediate operand and address widths through the context", () => {
    const values = new Map<number, number>([
      [0x00000100, 0x83],
      [0x00000101, 0xc0],
      [0x00000102, 0x02],
    ...
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0xabcd0003 }, eip: 0x00000103 });

    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0xabcd0002 }, eip: 0x00000107 });
  ...
  it("selects byte ModR/M ALU address width through the context", () => {
    const values = new Map<number, number>([
      [0x00000100, 0x67],
      [0x00000101, 0x00],
      [0x00000102, 0x08],
  ...
  it("selects XCHG operand and address widths through the context", () => {
    const values = new Map<number, number>([
      [0x00000100, 0x87],
      [0x00000101, 0xc8],
      [0x00000102, 0x66],
    ...
    state.loadProtectedModeSegment("ds", 0x0010, 0, 0xffffffff, true);
    state.writeRegister(0, 0xabcd0001);
    state.writeRegister(1, 0x12340002);
    state.writeRegister(3, 0xabcd3010);
    state.writeRegister(6, 0x56780000);

    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x12340002, ecx: 0xabcd0001 } });

    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x12340001, ecx: 0xabcd0002 } });

    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot().registers.ecx).toBe(0x12345678);
    expect([values.get(0x00003010), values.get(0x00003011), values.get(0x00003012)]).toEqual([
      0x02, 0x00, 0xcd
    ]);
    expect(state.snapshot().eip).toBe(0x00000108);
  ...
  it("repeats LODS through a contextual source-segment override", () => {
    const values = new Map<number, number>([
      [0x00000100, 0x64],
      [0x00000101, 0xf3],
      [0x00000102, 0xac],
    ...
    expect(state.snapshot()).toMatchObject({
      registers: { eax: 0x22, ecx: 0, esi: 0x2002 },
      eip: 0x103
    });
  });
  ...
  it("repeats MOVS through a contextual source-segment override", () => {
    ...
    expect([values.get(0x3000), values.get(0x3001)]).toEqual([0x11, 0x22]);
    expect(state.snapshot()).toMatchObject({
  ...
  it("selects MOVS and STOS data and index width independently through the context", () => {
    ...
    stepInstruction(resetAliasMemory(values), state);
    expect(Array.from({ length: 4 }, (_, offset) => values.get(0x00013000 + offset))).toEqual([
      0x11, 0x22, 0x33, 0x44
    ]);
    expect(state.snapshot()).toMatchObject({ registers: { esi: 0x00012004, edi: 0x00013004 } });

    state.writeRegister(6, 0x00012000);
    state.writeRegister(7, 0x00013010);
    stepInstruction(resetAliasMemory(values), state);
    expect([values.get(0x00013010), values.get(0x00013011)]).toEqual([0x11, 0x22]);
    expect(state.snapshot()).toMatchObject({ registers: { esi: 0x00012002, edi: 0x00013012 } });

    state.writeRegister(6, 0xabcd2000);
    state.writeRegister(7, 0xdcba3000);
    stepInstruction(resetAliasMemory(values), state);
    expect(Array.from({ length: 4 }, (_, offset) => values.get(0x00003000 + offset))).toEqual([
      0xaa, 0xbb, 0xcc, 0xdd
    ]);
    expect(state.snapshot()).toMatchObject({ registers: { esi: 0xabcd2004, edi: 0xdcba3004 } });

    state.writeRegister(0, 0x88776655);
    state.writeRegister(7, 0x00014000);
    stepInstruction(resetAliasMemory(values), state);
    expect(Array.from({ length: 4 }, (_, offset) => values.get(0x00014000 + offset))).toEqual([
      0x55, 0x66, 0x77, 0x88
    ]);
    expect(state.snapshot().registers.edi).toBe(0x00014004);
  });

  it("selects CMPS and SCAS data and index width independently through the context", () => {
    const values = new Map<number, number>([
      [0x00000100, 0xa7],
      [0x00000101, 0x66],
      [0x00000102, 0xa7],
    ...
    expect(state.snapshot()).toMatchObject({
      eflags: 0x00000046,
      registers: { esi: 0x00012004, edi: 0x00013004 }
    });
    ...
    expect(state.snapshot()).toMatchObject({
      eflags: 0x00000046,
      registers: { esi: 0x00012002, edi: 0x00013002 }
    });
    ...
    expect(state.snapshot()).toMatchObject({
      eflags: 0x00000046,
      registers: { esi: 0xabcd2004, edi: 0xdcba3004 }
    });

    state.writeRegister(0, 0xddccbbaa);
    state.writeRegister(7, 0x00003000);
    stepInstruction(resetAliasMemory(values), state);
  ...
  it("returns from a same-privilege protected-mode 16-bit far call", () => {
    const values = new Map<number, number>([
      [0x00000000, 0x9a],
      [0x00000001, 0x34],
      [0x00000002, 0x12],
  ...
  it("jumps through a protected-mode 32-bit memory far pointer", () => {
    const values = new Map<number, number>([
      [0x00000000, 0x66],
      [0x00000001, 0xff],
      [0x00000002, 0x2e],
  ...
  it("delivers protected-mode CLI and STI privilege faults with a zero error code", () => {
    for (const opcode of [0xfa, 0xfb]) {
      ...
      expect(state.snapshot()).toMatchObject({
        eip: 0x00000056,
        cs: { selector: 0x000b, default32: true },
        registers: { esp: 0x2ff0 },
        eflags: 0x00000002
      });
      expect(Array.from({ length: 16 }, (_, offset) => values.get(0x2ff0 + offset))).toEqual([
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x02, 0x02, 0x00,
        0x00
      ]);
    }
  });

  it("delivers protected-mode CLTS and control-register privilege faults", () => {
    for (const instruction of [
      [0x0f, 0x06],
      [0x0f, 0x22, 0xc0]
    ]) {
      const values = new Map<number, number>([
        ...instruction.map((value, index) => [index, value] as const),
        [0x1008, 0xff],
        [0x1009, 0xff],
        [0x100a, 0x00],
        [0x100b, 0x00],
  ...
  it("delivers protected-mode descriptor-table and machine-status privilege faults", () => {
    for (const instruction of [
      [0x0f, 0x01, 0x16, 0x00, 0x20],
      [0x0f, 0x01, 0x1e, 0x00, 0x20],
      [0x0f, 0x01, 0xf0],
      [0x66, 0x0f, 0x01, 0x16, 0x00, 0x20]
    ]) {
      const values = new Map<number, number>([
        ...instruction.map((value, index) => [index, value] as const),
        [0x1008, 0xff],
      ...
      expect(state.snapshot()).toMatchObject({
        ...
        registers: { esp: 0x2ff0 },
        cr0: 0x00000001,
        gdtr: { base: 0x1000, limit: 0x000f },
        idtr: { base: 0x2000, limit: 0x006f }
      });
      expect(Array.from({ length: 4 }, (_, offset) => values.get(0x2ff0 + offset))).toEqual([
        0x00, 0x00, 0x00, 0x00
      ]);
  ...
  it("delivers protected-mode POPF and POPFD privilege faults before consuming the stack", () => {
    for (const instruction of [[0x9d], [0x66, 0x9d]]) {
      ...
      expect(Array.from({ length: 4 }, (_, offset) => values.get(0x3000 + offset))).toEqual([
        0xff, 0xff, 0xff, 0xff
      ]);
    }
  });
  ...
  it("uses CS defaults and 67 for SETcc effective addresses", () => {
    const values = new Map<number, number>([
      [0x0100, 0x0f],
      [0x0101, 0x94],
      [0x0102, 0x05],
    ...
    stepInstruction(memory, state);
    expect(values.get(0x12000)).toBe(1);
    expect(state.snapshot().eip).toBe(0x0107);
    stepInstruction(memory, state);
    expect(values.get(0x1234)).toBe(0);
  ...
  it("keeps PUSH r/m data width separate from stack and ModR/M address widths", () => {
    const values = new Map<number, number>([
      [0x0100, 0xff],
      [0x0101, 0x35],
      [0x0102, 0x00],
    ...
    stepInstruction(memory, state);
    expect([values.get(0x1ffc), values.get(0x1fff)]).toEqual([0x78, 0x12]);
    expect(state.snapshot()).toMatchObject({ registers: { esp: 0x1ffc }, eip: 0x0106 });
    stepInstruction(memory, state);
    expect([values.get(0x1ffa), values.get(0x1ffb)]).toEqual([0xef, 0xbe]);
    expect(state.snapshot()).toMatchObject({ registers: { esp: 0x1ffa }, eip: 0x010c });
  });

  it("uses CS defaults and 66 for accumulator-register XCHG", () => {
    const values = new Map<number, number>([
      [0x0100, 0x93],
      [0x0101, 0x66],
      [0x0102, 0x91]
    ...
    state.writeRegister(1, 0x76544321);

    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({
      registers: { eax: 0xabcdef01, ebx: 0x12345678 },
      eip: 0x0101
    });
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({
      registers: { eax: 0xabcd4321, ecx: 0x7654ef01 },
      eip: 0x0103
    });
  });

  it("uses CS defaults and independent 66/67 overrides for BOUND", () => {
    const values = new Map<number, number>([
      [0x0100, 0x62],
      [0x0101, 0x05],
      [0x0102, 0x00],
  ...
  it("uses CS and SS defaults independently for LEAVE", () => {
    const values = new Map<number, number>([
      [0x0100, 0xc9],
      [0x0101, 0x66],
      [0x0102, 0xc9],
    ...
    state.loadProtectedModeCodeSegment(0x0008, 0, 0xffffffff, 0x0100, true);
    state.loadProtectedModeSegment("ss", 0x0010, 0, 0xffffffff, true);
    state.writeRegister(5, 0x1200);
    const memory = resetAliasMemory(values);

    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({
      registers: { ebp: 0x12345678, esp: 0x1204 },
      eip: 0x0101
    });
    state.writeRegister(5, 0xabcd1300);
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({
      registers: { ebp: 0xabcdbef0, esp: 0xabcd1302 },
      eip: 0x0103
    });
  });
  ...
  it("uses contextual address size for system selector instructions", () => {
    const values = new Map<number, number>([
      [0x0100, 0x0f],
      [0x0101, 0x00],
      [0x0102, 0xd0],
    ...
    stepInstruction(memory, state);
    expect([values.get(0x1234), values.get(0x1235)]).toEqual([0x10, 0x00]);
  ...
  it("uses contextual operand and address sizes for memory bit-test operations", () => {
    const values = new Map<number, number>([
      [0x0100, 0x0f],
      [0x0101, 0xab],
      [0x0102, 0x0d],
    ...
    state.loadProtectedModeCodeSegment(0x0008, 0, 0xffffffff, 0x0100, true);
    state.loadProtectedModeSegment("ds", 0x0010, 0, 0xffffffff, true);
    state.writeRegister(1, 33);
    state.writeRegister16(3, 15);
    const memory = resetAliasMemory(values);

    stepInstruction(memory, state);
    expect([values.get(0x4004), values.get(0x4005)]).toEqual([0x02, 0x00]);
    expect(state.snapshot().eip).toBe(0x0107);
    stepInstruction(memory, state);
    expect([values.get(0x1234), values.get(0x1235)]).toEqual([0x00, 0x80]);
    expect(state.snapshot().eip).toBe(0x010e);
  });

  it("uses contextual operand and address sizes for immediate bit-test operations", () => {
    const values = new Map<number, number>([
      [0x0100, 0x0f],
      [0x0101, 0xba],
      [0x0102, 0x2d],
    ...
    expect([
      ...
      values.get(0x4002),
      values.get(0x4003)
    ]).toEqual([0x00, 0x00, 0x00, 0x80]);
    expect(state.snapshot().eip).toBe(0x0108);
    stepInstruction(memory, state);
    expect([values.get(0x1234), values.get(0x1235)]).toEqual([0x00, 0x00]);
    expect(state.snapshot().eip).toBe(0x0110);
  });

  it("uses contextual operand sizes for logical Group 2 shifts", () => {
    const values = new Map<number, number>([
      [0x0100, 0xc1],
      [0x0101, 0xe0],
      [0x0102, 0x04],
    ...
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x23456780 }, eip: 0x0103 });
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x234533c0 }, eip: 0x0106 });
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x0234533c }, eip: 0x0108 });
  });

  it("uses contextual operand sizes for arithmetic Group 2 shifts", () => {
    const values = new Map<number, number>([
      [0x0100, 0xc1],
      [0x0101, 0xf8],
      [0x0102, 0x04],
    ...
    state.writeCr0(0x00000001);
    state.loadProtectedModeCodeSegment(0x0008, 0, 0xffffffff, 0x0100, true);
    state.writeRegister(0, 0x87654321);
    state.writeRegister8(1, 4);
    const memory = resetAliasMemory(values);

    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0xf8765432 }, eip: 0x0103 });
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0xf8762a19 }, eip: 0x0106 });
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0xff8762a1 }, eip: 0x0108 });
  });

  it("uses contextual operand sizes for Group 2 ROL", () => {
    const values = new Map<number, number>([
      [0x0100, 0xc1],
      [0x0101, 0xc0],
      [0x0102, 0x04],
    ...
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x23456781 }, eip: 0x0103 });
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x2345cf02 }, eip: 0x0106 });
    stepInstruction(memory, state);
  ...
  it("rotates 8-bit registers through carry by one", () => {
    ...
    expect(rcrState.snapshot()).toMatchObject({ registers: { eax: 0 }, eflags: 0x0003 });
  });
  ...
  it("selects F7 data width from the contextual CS default", () => {
    const values = new Map<number, number>([
      [0x00000000, 0xf7],
      [0x00000001, 0xd3],
      [0x00000002, 0x66],
    ...
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { ebx: 0xedcba987 }, eip: 2 });

    state.writeRegister(3, 0x12345678);
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { ebx: 0x1234a987 }, eip: 5 });

    stepInstruction(memory, state);
    expect(values.get(0x00002000)).toBe(0x87);
    expect(values.get(0x00002001)).toBe(0xa9);
    expect(state.snapshot().eip).toBe(11);
  });

  it("executes contextual FE byte increment and decrement through 67 addressing", () => {
    const values = new Map<number, number>([
      [0x00000000, 0xfe],
      [0x00000001, 0xc0],
      [0x00000002, 0x67],
    ...
    stepInstruction(memory, state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 0x80 }, eflags: 0x892, eip: 2 });
  ...
  it("routes RCL and RCR through contextual operand and address sizes", () => {
    const values = new Map<number, number>([
      [0x00000000, 0xd1],
      [0x00000001, 0xd0],
      [0x00000002, 0x66],
    ...
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 1 }, eflags: 0x00000803, eip: 2 });

    state.writeRegister(0, 0x12340001);
    state.writeEflags(0x00000003);
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({
      registers: { eax: 0x12348000 },
      eflags: 0x00000803,
      eip: 5
    });

    state.writeRegister(0, 0x80000000);
    state.writeEflags(0x00000803);
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({ registers: { eax: 3 }, eflags: 0x00000802, eip: 8 });

    state.writeRegister(0, 1);
    state.writeRegister8(1, 1);
    state.writeEflags(0x00000003);
    stepInstruction(resetAliasMemory(values), state);
    expect(state.snapshot()).toMatchObject({
    ...
    state.writeRegister(5, 0x00002000);
    state.writeEflags(0x00000003);
    stepInstruction(resetAliasMemory(values), state);
    expect(Array.from({ length: 4 }, (_, offset) => values.get(0x00002000 + offset))).toEqual([
      3, 0, 0, 0
    ]);
