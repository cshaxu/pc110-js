...
export interface TaskRegisterState {
  readonly selector: number;
  readonly base: number;
  readonly limit: number;
  readonly default32: boolean;
...
function cloneTaskRegister(taskRegister: TaskRegisterState): TaskRegisterState {
  return { ...taskRegister };
}
...
export class Cpu386State {
  ...
  public writeDoubleShiftFlags32(value: number, carry: boolean): void {
  ...
  public writeShiftLeftFlags32(value: number, count: number): void {
    const source = value >>> 0;
    const normalizedCount = count & 0x1f;
    if (normalizedCount === 0) return;
    const carry = source << (normalizedCount - 1);
    const result = (carry << 1) >>> 0;
    let flags = this.eflags & ~EFLAGS_LOGIC_MASK;
    if (carry & 0x80000000) flags |= EFLAGS_CARRY;
    if (result === 0) flags |= EFLAGS_ZERO;
    if (result & 0x80000000) flags |= EFLAGS_SIGN;
    if (((result & 0xff).toString(2).replace(/0/g, "").length & 1) === 0) flags |= EFLAGS_PARITY;
    if ((result ^ carry) & 0x80000000) flags |= EFLAGS_OVERFLOW;
    this.eflags = (flags | RESET_EFLAGS) >>> 0;
  }
  ...
  public writeRotateRightFlags8(result: number, count: number): void {
    const value = result & 0xff;
    let flags = this.eflags & ~EFLAGS_CARRY;
    if (value & 0x80) flags |= EFLAGS_CARRY;
    if (count === 1) {
      flags &= ~EFLAGS_OVERFLOW;
  ...
  public writeRotateFlags32(result: number, carry: boolean): void {
    let flags = this.eflags & ~(EFLAGS_CARRY | EFLAGS_OVERFLOW);
    if (carry) flags |= EFLAGS_CARRY;
    if (Boolean(result & 0x80000000) !== carry) flags |= EFLAGS_OVERFLOW;
    this.eflags = (flags | RESET_EFLAGS) >>> 0;
  }

  public writeRotateRightFlags16(result: number): void {
    const value = result & 0xffff;
    let flags = this.eflags & ~(EFLAGS_CARRY | EFLAGS_OVERFLOW);
    if (value & 0x8000) flags |= EFLAGS_CARRY;
    if (Boolean(value & 0x8000) !== Boolean(value & 0x4000)) flags |= EFLAGS_OVERFLOW;
    this.eflags = (flags | RESET_EFLAGS) >>> 0;
  }

  public writeRotateRightFlags32(result: number): void {
    const value = result >>> 0;
    let flags = this.eflags & ~(EFLAGS_CARRY | EFLAGS_OVERFLOW);
    if (value & 0x80000000) flags |= EFLAGS_CARRY;
    if (Boolean(value & 0x80000000) !== Boolean(value & 0x40000000)) flags |= EFLAGS_OVERFLOW;
    this.eflags = (flags | RESET_EFLAGS) >>> 0;
  }
  ...
  public writeShiftRightFlags32(value: number, count: number): void {
    const source = value >>> 0;
    const normalizedCount = count & 0x1f;
    if (normalizedCount === 0) return;
    const carry = source >>> (normalizedCount - 1);
    ...
    if (result & 0x80000000) flags |= EFLAGS_SIGN;
    if (((result & 0xff).toString(2).replace(/0/g, "").length & 1) === 0) flags |= EFLAGS_PARITY;
    if (result & 0x80000000) flags |= EFLAGS_OVERFLOW;
    this.eflags = (flags | RESET_EFLAGS) >>> 0;
  }

  public writeArithmeticShiftRightFlags16(value: number, count: number): void {
    const source = value & 0xffff;
    const normalizedCount = count & 0x1f;
    if (normalizedCount === 0) return;
    const carry = source >>> (normalizedCount - 1);
    const result = (((source << 16) >> 16) >> normalizedCount) & 0xffff;
    let flags = this.eflags & ~EFLAGS_LOGIC_MASK;
    if (carry & 0x01) flags |= EFLAGS_CARRY;
    if (result === 0) flags |= EFLAGS_ZERO;
    if (result & 0x8000) flags |= EFLAGS_SIGN;
  ...
  public writeArithmeticShiftRightFlags32(value: number, count: number): void {
    const source = value | 0;
    const normalizedCount = count & 0x1f;
    if (normalizedCount === 0) return;
    const carry = source >>> (normalizedCount - 1);
    const result = source >> normalizedCount;
    let flags = this.eflags & ~EFLAGS_LOGIC_MASK;
    if (carry & 0x01) flags |= EFLAGS_CARRY;
    if (result === 0) flags |= EFLAGS_ZERO;
    if (result & 0x80000000) flags |= EFLAGS_SIGN;
